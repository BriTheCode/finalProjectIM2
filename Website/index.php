...
if (isset($_POST['submit'])) {
  ...
  if (!empty($employeename) && !empty($employeeemail)) {
      // Check if the employee (user_name or email) already exists
      $checkQuery = "SELECT * FROM registration WHERE user_name = '$employeename' OR email = '$employeeemail'";
      $checkResult = mysqli_query($connect, $checkQuery);

      if (mysqli_num_rows($checkResult) > 0) {
          // Duplicate employee found
          $alerting = '<div id="delete-alert" class="alert alert-warning" role="alert">Employee with this name or email already exists!</div>';
      } else {
          // If no duplicate found, insert the employee
          $insertQuery = "INSERT INTO registration (user_name, email) VALUES ('$employeename','$employeeemail')";
          
          if (mysqli_query($connect, $insertQuery)) {
              $alerting = '<div id="success-alert" class="alert alert-success success-alert" role="alert">Employee added successfully!</div>';

              // Insert the department if provided
              if (!empty($departmentname) && !empty($location)) {
                  $userid = mysqli_insert_id($connect); // Get the last inserted user_id
                  $insertDeptQuery = "INSERT INTO departments (user_id, department_name, location) VALUES ('$userid', '$departmentname', '$location')";

                  if (mysqli_query($connect, $insertDeptQuery)) {
                      $alerting = '<div id="success-alert" class="alert alert-success success-alert" role="alert">Employee and department added successfully!</div>';
                  } else {
                      $alerting = '<div id="delete-alert" class="alert alert-danger" role="alert">Error adding department. Please try again.</div>';
                  }
              }
          } else {
              $alerting = '<div id="delete-alert" class="alert alert-danger" role="alert">Error adding employee. Please try again.</div>';
          }
      }
  } else {
      $alerting = '<div id="delete-alert" class="alert alert-warning" role="alert">Employee name and email cannot be empty!</div>';
  }
}
          ...
          <div class="col-md-12 mb-3">
            <div class="card">
              <div class="card-header">
                <span><i class="bi bi-table me-2"></i></span> Data Table Employee
...
<!-- ENDING ADDING EMPLOYEE WITH MODAL -->
    ...
    <thead>
        ...
        <tbody>
          <?php
            ...
            if ($search != "") {
              $retrieve = "SELECT * FROM registration WHERE user_name LIKE '%$search%'";
          } else {
              $retrieve = "SELECT * FROM registration";
          }
          ...
          while ($rows = mysqli_fetch_assoc($result)){
            ...
            <tr>
                ...
                <td><a href="#" class="btn btn-success update-btn" data-id="<?php echo $rows['user_id']; ?>" data-name="<?php echo $rows['user_name']; ?>" data-email="<?php echo $rows['email']; ?>" data-bs-toggle="modal" data-bs-target="#updateModal">Update</a></td>
                <td><a href="delete.php?user_id=<?php echo $rows['user_id']; ?>" class="btn btn-danger">Delete</a></td>
            </tr>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="" method="post">
          ...
          <div class="form-group">
            <label for="employeename">User Name:</label>
            <input type="text" id="employeename" name="e_name" class="form-control" required>
          </div>
          <div class="form-group">
            <label for="employeeemail">Email:</label>
            <input type="email" id="employeeemail" name="e_email" class="form-control" required>
          </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
